  ...
  constructor(scope: Construct, id: string, buildConfig: BuildConfig, props?: cdk.StackProps)
    {
      ...
      const myLambda = new Function(this, (buildConfig.prefix + "-" + buildConfig.stage_dev), {
        runtime: Runtime.NODEJS_12_X, //using node for this, but can easily use python or other
        handler: 'handler.handler',
        code: Code.fromAsset(path.join(__dirname, 'lambda')), //resolving to ./lambda directory
      ...
      const findlambdaName = new cdk.CfnOutput(this, "lambdaOutput", {
      value: myLambda.functionName,
      exportName: "myLambdaName",
      });

      console.log ("lambda name " + findlambdaName.value  + findlambdaName. exportName)
    }
}